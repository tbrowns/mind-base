...
export async function processIngestionJob(jobId: string) {
  ...
  try {
    const document = await ingestDocument({ title: job.title, description: job.sourceType === "gmail" ? "Imported from the Kuzana Brain Inbox." : "Imported meeting transcript or notes.", text: job.rawText, accessLevel: job.accessLevel, sourceType: job.sourceType, metadata: job.metadata });
